...
// Always returns 2 best edges (unless there are <2 fixtures), regardless of threshold.
// Set ?strict=1 to hard-enforce minConf; by default it's best-effort top2.
...
// European scope + cups (UCL/UEL/UECL)
...
// 1st/2nd divisions + cups in Europe
...
// Simple prior helps early season be reasonable
...
module.exports = async (req, res) => {
  try{
    ...
    const minConf  = Number(req.query.minConf || DEFAULT_MIN_CONF); // label
    const strict   = req.query.strict === '1'; // only if you want hard filter
    const debug    = req.query.debug  === '1';
    const refresh  = req.query.refresh=== '1'; // accepted but no cache here

    // 1) fixtures (Europe + cups)
    const fixtures = (await fetchAllEuropeFixtures(date)).slice(0, MAX_FIXTURES_TO_SCORE);
    ...
    // if strict filter yields <2, fall back to top2 overall so page never empties
