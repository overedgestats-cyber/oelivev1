...
(function(){
  ...
  const MANUAL_RESULTS = [
    ...
    { date:"2025-10-01", home:"Dortmund W", away:"Bayern Munich W", market:"Over 2.5", odds:2.00, status:"lose" },
    { date:"2025-10-01", home:"Utsikten",    away:"IK Brage",       market:"Over 2.5", odds:1.67, status:"win" },

    // previous seed (keep your data here) ...
  ...
  (async function init(){
    const remote = await fetchRemote(180); // will be [] on 500
    ...
    // ===== Summary KPIs (three rows of cards) =====
    ...
    setText('s-ytd-win', fmtPct(yStats.winRate));
    setText('s-ytd-roi', fmtPct(yStats.roiPct));
    ...
    // CLV requires closing odds, which we may not have; leave as '—' if absent.
    ...
    // ===== Month drop-down & list (optional: keep as-is if you use it) =====
    // (omitted here for brevity; your previous code works unchanged)
  })();
})();
